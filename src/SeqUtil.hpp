...
#define MAJOR_VERS 0
#define MINOR_VERS 2
#define PREALIGN_NUM 10000
//#define DEBUG    //Turn on to output time consuming of three parts of alignment
...
int getAlignInfoSE(char *seq, bwtintv_v *a, bwtintv_v *tmpvec[2], smem_i *func_itr, bwaidx_t *func_idx,
                   align_info &align_info1, char *degenerate) {
#ifdef DEBUG
        ...
        if (pvec[i].x[2] <= max_iwidth) {
            for (int k = 0; k < pvec[i].x[2]; ++k) {
                ...
                if (is_rev) {
                    ...
                    free(rseq_l);
                    if (missum <= max_mis) {
                        rseq_r = bns_get_seq(func_idx->bns->l_pac, func_idx->pac,
                                             pos + seql - (uint32_t) (pvec[i].info >> 32),
                                             pos + seql, &rlen);
...
int getAlignInfoPE(char *seq1, char *seq2, bwtintv_v *a1, bwtintv_v *a2, bwtintv_v *tmpvec[2], smem_i *func_itr,
                   bwaidx_t *func_idx,
                   align_info &align_info1, align_info &align_info2, int64_t **idx1, int64_t **idx2,
                   char (*degenerate)[512]) {
    int64_t rlen;
    ...
    if (isSE) {
        ...
        for (i = 0; i < PREALIGN_NUM; i++) {
            if (ssread1.getRead() && ssread2.getRead()) {
                ...
                seqm = getAlignInfoPE(ssread1.seq, ssread2.seq, matcher1, matcher2, NULL, itr, idx, align_info1,
                                      align_info2, idx1, idx2, degenerate);
                if (seqm > 0) ++alignedNum;
            } else {
                prealign_num = i;
                break;
...
uint32_t parse_decode_buf(char *pbuf, char *namebuf, char *seqbuf, char *qualbuf, uint16_t *seqlen, int count) {
    ...
    for (int k = 0; k < count; k++) {
        while ((*pbuf++ = *namebuf++) != '\n'); //id
        memcpy(pbuf, seqbuf, seqlen[k]);
        seqbuf += seqlen[k];
        pbuf += seqlen[k];//seq
        *pbuf++ = '\n';
        *pbuf++ = '+';
        *pbuf++ = '\n';
        memcpy(pbuf, qualbuf, seqlen[k]);
        qualbuf += seqlen[k];
        pbuf += seqlen[k];//qual
        *pbuf++ = '\n';
    }
...
void decode_from_bitarry(BitDecode &bitdecode, int quallen, uint8_t order, bwaidx_t *pidx, char *outbuf,
                         std::vector<char>::iterator &itor, bool isread2 = false) {
    order -= 1;
    ...
    while (true) {
        ...
        for (int k = 0; k < count; k++) {
            ...
            if (orderbuf[k] > 0) {
                ...
                memcpy(&write_buf[out_ind], seqbuf, quallen[k]);
                seqbuf += quallen[k];
                out_ind += quallen[k];
            }
            ...
            {
                memcpy(&write_buf[out_ind], pname + 1, id_len);
                out_ind += id_len;
            } else {
                write_buf[out_ind++] = '\n';
            }
            memcpy(&write_buf[out_ind], qualbuf, quallen[k]);
            qualbuf += quallen[k];
            out_ind += quallen[k];
            write_buf[out_ind++] = '\n';
        }
        ...
        } else //分别写入f1和f2
        {
            ...
            for (int k = 0; k < count; k++) {
                ...
                if (orderbuf[k] > 0) {
                    ...
                    memcpy(pbuf, buf, quallen[k]);
                    pbuf += quallen[k];//seq
                } else {
                    memcpy(pbuf, seqbuf, quallen[k]);
                    seqbuf += quallen[k];
                    pbuf += quallen[k];//seq
                }
...
bool GetFileSlice(char *path, uint64_t flength, uint64_t *slicearry) {
    ...
    for (int i = 0; i < thread_num - 1; ++i) {
        ...
        while (!bfind) {
            ...
            fgets(buf, 1024, fdna);
            exlen += strlen(buf);//第二行
            fgets(buf, 1024, fdna);
            exlen += strlen(buf);//第三行
            if (buf[0] == '+') {
                bfind = true;
            }
            fgets(buf, 1024, fdna);
            exlen += strlen(buf);//第四行
        }
...
void MergeFileForzip(string &fastq_prefix, int num, int index) {
    ...
    sprintf(fastq_path, "./%s_%d.fastq", fastq_prefix.c_str(), index);
    fstream out_s;
