...
public:
  ...
  char* endSegmentChecksum(int64_t* size = 0, bool dosha1=false);
...
public:
  ...
  void read_buffer(StringBuffer& s, int n) {
    lengthen(n);
    memcpy(p+wpos, s.p+s.rpos, n);
    wpos += n;
    s.rpos += n;
  }
...
void compress(Reader* in, Writer* out, const char* method,
     const char* filename=0, const char* comment=0, bool dosha1=false);
